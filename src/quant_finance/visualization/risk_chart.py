...
"""Risk charts — drawdown, rolling metrics, and returns histogram with VaR."""
...
def plot_rolling_sharpe(
    ...
    for i in range(window, len(daily_returns) + 1):
        ...
        std = variance ** 0.5
        sharpe = ((mean_ret - rf_daily) / std * (252 ** 0.5)) if std > 0 else 0.0
        rolling_sharpes.append(sharpe)
        x_values.append(i - 1)

    fig, ax = plt.subplots(figsize=cfg.figsize)
    ax.plot(x_values, rolling_sharpes, color="#2196F3", linewidth=1.2)
    ax.axhline(0, color="black", linewidth=0.8, alpha=0.5)
    ax.fill_between(x_values, 0, rolling_sharpes,
...
def plot_rolling_volatility(
    ...
    """Rolling annualised volatility over a lookback window.
    ...
    for i in range(window, len(daily_returns) + 1):
        ...
        variance = sum((r - mean_ret) ** 2 for r in window_rets) / len(window_rets)
        std = variance ** 0.5
        vol_ann = std * (252 ** 0.5) * 100
        rolling_vols.append(vol_ann)
        x_values.append(i - 1)

    fig, ax = plt.subplots(figsize=cfg.figsize)
    ax.plot(x_values, rolling_vols, color="#FF9800", linewidth=1.2)
    ax.fill_between(x_values, 0, rolling_vols, color="#FF9800", alpha=0.2)
...
def plot_rolling_drawdown(
    ...
    fig, ax = plt.subplots(figsize=cfg.figsize)
    ax.plot(x_values, rolling_dds, color="#9C27B0", linewidth=1.2)
    ax.fill_between(x_values, 0, rolling_dds, color="#9C27B0", alpha=0.2)
    ax.axhline(0, color="black", linewidth=0.8, alpha=0.5)
    cfg.apply(ax)
    fig.tight_layout()
    return fig
...
def plot_drawdown(
    ...
    Args:
        navs: list of NAV values
        dates: optional dates aligned with navs. Defaults to index.
        config: optional ChartConfig
    """
    use_style()
